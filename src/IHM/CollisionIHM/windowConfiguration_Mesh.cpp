...
void WindowConfiguration_Mesh::loadFile()
{
	QStringList type_load;
 	...
 	if (filename != ""){
		...
		vector<Triangle> faces = mesh.getFaces();
		Vector3 C(0,0,0);
		for(uint i=0;i<mesh.getNbFaces();i++){
			for(uint j=0;j<3;j++)
				C = C + faces[i].getVertex(j);
		}
		C = C/(mesh.getNbFaces()*3);
		printf("centroid:%f %f %f\n",C.x(),C.y(),C.z());
		for(uint i=0;i<mesh.getNbFaces();i++){
			Triangle T = faces[i];
			Vector3 N = T.getNormale();
			Vector3 A = T.getVertex(0);
			Vector3 AV = C-A;
			double dot = AV.x()*N.x()+AV.y()*N.y()+AV.z()*N.z();
			if(dot>=0){
