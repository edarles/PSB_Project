...
bool SurfaceSPH::extract(System *S, double isoLevel, uint step)
{
	...
	nbPosX = ceil(l/sizeCell);
	nbPosY = ceil(w/sizeCell);
	nbPosZ = ceil(d/sizeCell);
	...
	// CPU structures
	/*******************************************************************************/
	double *vertexs, *vertex2, *normales, *normales2;
	int *indexs, *nbIndex;
...
/***********************************************************************************************/
/***********************************************************************************************/
void SurfaceSPH::drawGrid()
{
        glPolygonMode(GL_FRONT_AND_BACK,GL_LINE);
	float s = sizeCell;
	for(unsigned int i=0;i<nbPosX;i++){
		for(unsigned int j=0;j<nbPosY;j++){
			for(unsigned int k=0;k<nbPosZ;k++){
				uint indexCell = i + j*nbPosX + k*nbPosX*nbPosY;
				if(grid[indexCell*4+3]>1){
				double xC = grid[indexCell*4];
				double yC = grid[indexCell*4+1];
				double zC = grid[indexCell*4+2];
...
bool SurfaceSPH::exportOBJ(const char* filename)
{
	if(indexs_cpu.size()>0){
		...
		/*ofstream f1;
		f1.open("fluid.mtl");
		f1 << "# Blender3D MTL File: <memory>" << endl;
		f1 << "# Material Count: 1" << endl;
		f1 << "newmtl fluid_mtl" << endl;
		f1 << "Ka 0.000000 0.000000 0.000000" << endl;
		f1 << "Kd 0.8 0.8 0.8" << endl;
		f1 << "Ks 0.5 0.5 0.5"<< endl;
		f1 << "Ns 100" << endl;
		f1 << "Ni 0.000000" << endl;
		f1.close();
*/
		...
		ofstream f;
		f.open(filename);
		//f << "# Blender v2.62 (sub 0) OBJ File: ''" << endl;
		//f << "# www.blender.org" << endl;
		//f << "mtllib fluid.mtl" << endl;
		//f << "o isosurface" << endl;

		for(unsigned long i=0;i<vertexs_cpu.size();i++)
			f << "v " <<  vertexs_cpu[i].x() << " " << vertexs_cpu[i].z() << " " <<vertexs_cpu[i].y() << endl;
		for(unsigned long i=0;i<normales_cpu.size();i++)
			f << "vn " << normales_cpu[i].x() << " " << normales_cpu[i].z() << " " << normales_cpu[i].y() << endl;
		//f << "usemtl fluid_mtl" << endl;
		//f << "s off" << endl;
		//f << "f" << endl;
		for(unsigned long i=0;i<indexs_cpu.size()-3;i+=3){
			...
			f << "f " << i1+1 << "//" << i1+1 << " " << i2+1 << "//" << i2+1 << " " << i3+1 << "//" << i3+1 << endl;
		}
		f.close();
		return true;
	}
