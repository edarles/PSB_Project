...
__device__ float calculate_density(float4 posA, float4 posB,
 ...
 if(dist<radiusA || dist<radiusB){
   float m_inv_2hSqr = 1./(2.*radiusA*radiusA);
   float m_k = 1./powf(2*3.14*radiusA*radiusA, 1.5);
   dens = (dist*dist)*m_inv_2hSqr;
   dens = m_k*exp(-dens);
 }
 return dens;
}
